...
class Starter_Templates_Export {
	function __construct( $args ) {
		...
		if ( $_GET['from_starter'] == 'placeholder' ) {
			/**
			 * @see export_wp
			 */
			update_option( 'starter_import_placeholder_only', true );
			$this->export_wp( $args );
			die();
        } else {
			update_option( 'starter_import_placeholder_only', null );
	...
	function the_content_export( $content ){
        return Starter_Templates_Placeholder::get_instance()->content_replace_placeholder( $content );
    }

	function wp_get_attachment_url( $url, $post_id = false ){
		return Starter_Templates_Placeholder::get_instance()->wp_get_attachment_url( $url, $post_id );
    }
	...
	function export_wp( $args = array() ) {
		...
		>
			...
			<channel>
				 ...
				 */
                if ( Starter_Templates_Placeholder::get_instance()->placeholder_post ) {
	                $post = Starter_Templates_Placeholder::get_instance()->placeholder_post;
	                setup_postdata( $post );
	                $this->post_item( $post );
                }

				if ( Starter_Templates_Placeholder::get_instance()->logo_post ) {
					$post = Starter_Templates_Placeholder::get_instance()->logo_post;
					setup_postdata( $post );
					$this->post_item( $post );
				}
	...
	function progress_elementor_data( $data ){
	    return Starter_Templates_Placeholder::get_instance()->progress_elementor_data( $data );
    }

	function progress_meta( $meta ){
        return Starter_Templates_Placeholder::get_instance()->progress_meta( $meta );
    }
...
function starter_templates_export_wp( $args = array() ){
	if ( isset( $_GET['download'], $_GET['content'], $_GET['from_starter'] ) ) {
		new Starter_Templates_Export( $args );
	}
}
add_action( 'export_wp', 'starter_templates_export_wp' );